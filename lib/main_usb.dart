...
class _EmotivHomePageState extends State<EmotivHomePage>
  ...
  void initState() {
    ...
    // Default to USB on Windows by starting USB scan instead of BLE
    if (Platform.isWindows) {
      _activeTransport = Transport.usb;
      _usbManager.statusStream.listen((s) => setState(() => _statusMessage = s));
      _usbManager.connectionStream.listen((c) => setState(() => _isConnected = c));
      _usbManager.foundDevicesStream.listen((devs) => setState(() => _foundDevices = devs));
      _usbManager.eegDataStream.listen(_onEEGData);
      _usbManager.motionDataStream.listen(_onMotionData);
      _usbManager.startScanning();
    }
  }
  ...
  void _onEEGData(List<double> data) {
    final now = DateTime.now();
    if (now.difference(_lastUiUpdate) >= _uiUpdateInterval) {
      _lastUiUpdate = now;
      if (mounted) {
  ...
  void _onMotionData(List<double> data) {
    final now = DateTime.now();
    if (now.difference(_lastUiUpdate) >= _uiUpdateInterval) {
      _lastUiUpdate = now;
      if (mounted) {
  ...
  Transport _activeTransport = Transport.ble;

  Future<void> _startScanning() async {
    if (_activeTransport == Transport.usb) {
      await _usbManager.startScanning();
    } else {
  ...
  Future<void> _stopScanning() async {
    if (_activeTransport == Transport.usb) {
      await _usbManager.stopScanning();
    } else {
  ...
  Future<void> _connectToDeviceByName(String deviceName) async {
    try {
      if (_activeTransport == Transport.usb) {
        await _usbManager.connectToDeviceByName(deviceName);
      } else {
  ...
  Future<void> _openFileSettings() async {
    ...
    if (result != null && result is String) {
      ...
      // Apply the new directory to active transport manager
      if (_activeTransport == Transport.usb) {
        _usbManager.setCustomSaveDirectory(_selectedDirectory);
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Emotiv LSL Logger'),
        backgroundColor: Theme.of(context).colorScheme.inversePrimary,
        actions: [
          // Transport toggle (USB on Windows default)
          if (Platform.isWindows)
            Padding(
              ...
              child: Row(
                children: [
                  const Text('BLE'),
                  Switch(
                    value: _activeTransport == Transport.usb,
                    onChanged: (val) async {
                      await _disconnect();
                      setState(() {
                        _foundDevices = [];
                        ...
                        _latestMotionData = [];
                        _eegRecords.clear();
                        _activeTransport = val ? Transport.usb : Transport.ble;
                      });
                      await _startScanning();
                    },
                  ),
        ...
        child: Column(
          ...
          children: [
            ...
            BluetoothControlWidget(
              ...
              isScanning: _activeTransport == Transport.usb ? _usbManager.isScanning : _bleManager.isScanning,
